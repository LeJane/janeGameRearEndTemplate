		...
		if(msg.State==200){
			...
			if(res && res != null){
				...
				for(var i=0;i<res.length;i++){
					Obj=['',res[i]['CarNo'],res[i]['AddOilSum'],res[i]['AddOilTotal']]
					ObjRes.push(Obj);
				}
//				res[0]['AddOilSum']=123
//				res[1]['AddOilSum']=130
...
//				res[1]['AddOilTotal']=126
				...
				//获取表格
				getTable('#yhtjbb',ObjRes,true,true,true,true);	
			}
		}else{
    ...
    if ($(this).hasClass("row-details-open")) //判断是否已打开
    {
	   ...
	   $(this).addClass("row-details-open").removeClass("row-details-close");
		...
		var SubmitData={
			...
			if(!DefaultStatus){
				ChildData(SubmitData,"#sigleCarFuelFill"+RowData[1]+"Table",'#sigleCarFuelFill'+RowData[1]+'',"/OilStatis/CarAddOilStatisResult");	//默认请求
				DefaultStatus=true;
			}
		...
		if(msg.State==200){
			...
			if(res){
				...
				if(field==''){
					...
					if(field="#sigleCarFuelFill"+carnos+""){
