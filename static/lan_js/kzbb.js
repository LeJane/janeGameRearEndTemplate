		...
		if(msg.State==200){
			...
			if(res && res != null){
				...
				for(var i=0;i<res.length;i++){
					...
					Obj=['',res[i]['CarNo'],res[i]['IdleThan'],res[i]['IdleTime'],res[i]['IdleOilExpendTotal']]
					ObjRes.push(Obj);
				}
//				res[0]['IdleThan']=12
//				res[1]['IdleThan']=22
//				res[0]['IdleTime']=2
//				res[1]['IdleTime']=3
//				res[0]['IdleOilExpendTotal']=45
//				res[1]['IdleOilExpendTotal']=60
				//报表生成
				chartsAddyou(res);
				if(DataTableThtj){
...
var chartsAddyou=function(param){
	...
	var IdleThan=new Array();   //空转百分比
	var IdleTime=new Array();   //空转时间
	var IdleOilExpendTotal=new Array();   //空转油耗
	for(var i=0;i<param.length;i++){
		Carno.push(param[i]['CarNo']);	//车辆信息
		IdleThan.push(param[i]['IdleThan']);	//空转百分比
		IdleTime.push(parseInt(param[i]['IdleTime']));	 //空转时间
		IdleOilExpendTotal.push(param[i]['IdleOilExpendTotal']);	 //空转油耗
	}
	var ser=[
				{
					"name":"空转百分比",
	...
	if ($(this).hasClass("row-details-open")) //判断是否已打开
   {
	   ...
	   /* Open this row */
		...
		var SubmitData={
			...
			if(!DefaultStatus){
				ChildData(SubmitData,"#sigleCarSpin"+RowData[1]+"Table",'#sigleCarSpin'+RowData[1]+'',"/OilStatis/CarIdelOilStatisResult");	//默认请求
				DefaultStatus=true;
			}
...
//				'<li><a id="DefaultsigleCarRun" href="#sigleCarRun'+ d +'"  data-RequestUrl="/OilStatis/CarIdelOilStatisResult"  data-toggle="tab">单车空转</a></li>'+
//			'</ul>'+
