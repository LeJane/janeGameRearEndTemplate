...
function updateMarker(carInfo) {
    ...
    if (marker == null) {
        ...
        var currentPoint = marker.getPosition();//当前坐标
        var starttime = marker.getExtData().GpsDateTime;//当前时间
        var endtime = info.GpsDateTime;//结束时间
	    var time = getTime(starttime, endtime);//计算时间差
	    updateInfo(marker, info); //更新点标记信息
	    getRoute(currentPoint, point,time);
		if(points.length>0){
...
//			address(pos)
	    	Str+="<tr class='CarNo"+ updateCarArry[acf]['CarNo'] +"'>"+"<td>"+ updateCarArry[acf]['CarNo'] +"<td>"+ updateCarArry[acf]['GpsDateTime'] +"</td>"+"<td>"+updateCarArry[acf]['AccState'] +"</td>"+"<td>"+ updateCarArry[acf]['Speed'] +".00"+"</td>"+"<td>"+ updateCarArry[acf]['Direction'] +"</td>"+"<td>"+ updateCarArry[acf]['GoogleLongitude'] +"</td>"+"<td>"+ updateCarArry[acf]['GoogleLatitude'] +"</td>"+"<td>绵阳</td>"+"</tr>";
			$('#CarNoData').html(Str);
		}
	}
}
