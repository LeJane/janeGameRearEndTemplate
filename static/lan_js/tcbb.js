		...
		if(msg.State==200){
			...
			if(res && res != null){
				...
				for(var i=0;i<res.length;i++){
					...
					Obj=['',res[i]['CarNo'],res[i]['StopThan'],res[i]['StopTime']]
					ObjRes.push(Obj);
...
var chartsAddyou=function(param){
	...
	var StopThan=new Array();   //停车百分比
	var StopTime=new Array();   //停车时间
	console.log(StopTime);
	for(var i=0;i<param.length;i++){
		Carno.push(param[i]['CarNo']);	//车辆信息
		StopThan.push(param[i]['StopThan']);	//停车百分比
		StopTime.push(parseInt(param[i]['StopTime']));	 //停车时间
    ...
    if ($(this).hasClass("row-details-open")) //判断是否已打开
    {
	   ...
	   $(this).addClass("row-details-open").removeClass("row-details-close");
		...
		var SubmitData={
			...
			if(!DefaultStatus){
				ChildData(SubmitData,"#sigleCarStop"+RowData[1]+"Table",'#sigleCarStop'+RowData[1]+'',"/OilStatis/CarStopStatisResult");	//默认请求
				DefaultStatus=true;
			}
...
function format ( d ) {
    return '<ul id="myTab" class="nav nav-tabs">'+
				'<li class="active"><a href="#sigleCarStop'+ d +'" data-RequestUrl="/OilStatis/CarStopStatisResult"  data-toggle="tab">单车停车</a></li>'+
			'</ul>'+
			'<div id="myTabContent" class="tab-content">'+
//单车停车				
			'<div class="tab-pane fade in active" id="sigleCarStop'+ d +'">'+
