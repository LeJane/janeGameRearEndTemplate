...
function SetWidth( Demo="0" ){
	//动态设置高度

	var windowHeight = $(window).height();		

	var headerHeight = $("#headerHeight").height();//顶部的高度

	// var titleHeight = $("#titleHeight").height() + 30;
	
	// var CentHeight=$("#CentHeight").height()
	if(Demo=="0"){
		var demoHeight = $("#ScrollTab").height();
		if(demoHeight >100){
			demoHeight=200;
		}
	}else{
	...
	$("#container").css({
		height:(TotalHeight-50)+"px"
	})
	$("#LeftHeight").css({
		"min-height":(TotalHeight+15)+"px",
		"max-height":(TotalHeight+15)+"px"
	})
}
...
$(function(){
	...
	function DataTableHistory(process){
		//表格交互
			if(FlagPro){
				
				IntHeight =IntHeight+1;
				
				var tableTrHeight = ($("#CarNoData tr:first-child").height()) * IntHeight;
