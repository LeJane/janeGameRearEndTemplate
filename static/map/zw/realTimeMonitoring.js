(function(window,$){
    ...
    tableHeight,mapHeight,newMapHeight,winHeight,headerHeight,dbclickCheckedId,oldDbclickCheckedId,onClickVId,oldOnClickVId,zTree;
	...
	//表头状态列表ID
	var $tableCarAll = $("#table-car-all");
	var $tableCarOnline = $("#table-car-online");
	var $tableCarOffline = $("#table-car-offline");
	var $tableCarRun = $("#table-car-run");
	var $tableCarStop = $("#table-car-stop");
	var $tableCarOnlinePercent = $("#table-car-online-percent");
	var runVidArray = [];//存放行驶车辆ID
	var stopVidArray = [];//存放停止车辆ID
    ...
    realTimeMonitoring = {
        init: function(){
            ...
            setting = {
                ...
                callback: {
                    ...
                    onRightClick: realTimeMonitoring.zTreeShowRightMenu,
                }
            };
            setTimeout(function () {
	            ...
	            $.ajax({
	                ...
	                success: function (data) {
                      ...
                      var  udiyueall=realTimeMonitoring.unique(diyueall)
                        ...
                        webSocket.subscribe(headers,'/user/'+ $("#userName").text() + '/cachestatus', realTimeMonitoring.updataRealTree,"/app/vehicle/subscribecachestatus", requestStrS);
	                    $("#tall").text("(" + udiyueall.length + ")");
	                    $tableCarAll.text(udiyueall.length);
	                },
	                error: function () {
        ...
        completeEventHandler:function(vehicle){//1
           ...
           ListeningMovement: function(plate){
             ...
             setTimeout(function () {
                ...
                //console.log(markerInside[0] +  "11111111111");
        ...
        zTreeOnAsyncSuccess: function(event, treeId, treeNode, msg){
            ...
            if(jumpId =="trackPlayer"){
                ...
                webSocket.subscribe(headers,'/user/'+ $("#userName").text() + '/location', realTimeMonitoring.updataRealTree,"/app/vehicle/location", requestStrS);
            }
        },
        //模糊查询
        ...
        */
        ...
        onClickV: function(e, treeId, treeNode){
              ...
              if(oldOnClickVId = onClickVId){
	      	  ...
	      	  realTimeMonitoring.tableHighlightBlue(nodeName);
        },
        //双击事件
        onDbClickV: function(e, treeId, treeNode){
            ...
            if(nodes[0].type=="assignment"||nodes[0].type=="group"){
                ...
                cancelList=[];
                webSocket.subscribe(headers,"/user/" + $("#userName").text() + "/location", realTimeMonitoring.updataRealTime,"/app/vehicle/location", requestStrS);
            }
        ...
        getChannel: function(fenceNode, showMap){
            if (fenceNode == null || fenceNode.length == 0||fenceNode[0].type!=='vehicle') {
                ...
                webSocket.subscribe(headers,"/user/" + $("#userName").text() + "/location", realTimeMonitoring.updataRealTime,"/app/vehicle/location", requestStr);
            }
        },
            ...
            if (data.desc.msgID == 39321) {
                ...
                for (var i = 0; i < position.length; i++) {
                	if(position[i].speed == 0){
                		stopVidArray.push(position[i].vehicleId);
                		carStopLine++;
                	}else{
                		runVidArray.push(position[i].vehicleId);
                		carRunLine++;
                	}
                	$tableCarRun.text(carRunLine);
                	$tableCarStop.text(carStopLine);
                ...
                $tableCarOnline.text(Vjiaoji.length);
                $tableCarOnlinePercent.text((Vjiaoji.length/(Vjiaoji.length+vmiss)*100).toFixed(2) + "%");
                $("#tmiss").text("(" + vmiss + ")");
                $tableCarOffline.text(vmiss);
                ...
                if(diyueall.isHas(upPosition[0].vehicleId)){
                    ...
                    $("#tline").text("(" + iValue + ")");
                    $("#tmiss").text("(" + jValue + ")");
                    $tableCarOnlinePercent.text((iValue/(iValue+jValue)*100).toFixed(2) + "%");
                    $tableCarOnline.text(iValue);
                    $tableCarOffline.text(jValue);
                }
                if(upPosition[0].speed == 0){
        ...
        updateTable: function(position){
           ...
           // var laglatObjct = {'lnglatXYs':lnglatXY};
           address_submit("POST",'/clbs/v/monitoring/getAddress',"json",false,laglatObjct,true,realTimeMonitoring.getAddressback);
            if(carAddress == "AddressNull" ){
                geocoder.getAddress(lnglatXY, function (status, result) {
                    ...
                    if(longitude != 0 && latitude!= 0){
                        var addressParticulars = realTimeMonitoring.getaddressParticulars(AddressNew,longitude,latitude);
                        address_submit("POST",'/clbs/v/monitoring/setAddress',"json",true,{"AddressNew" : addressParticulars},false,null);
                    }
                    //逆地理编码 end
        ...
        alarmInfoDataDbclick: function(){
        	...
        	$("#alarmTable tbody tr").unbind("click").bind("click",function(){
            	...
            	$("#alarmTable tbody tr").removeClass("tableHighlight-blue");
        ...
        tableHighlight: function(name){
        	...
        	$realTimeStateTableList.children("tbody").children("tr").removeClass("tableHighlight-blue");
      	  	$realTimeStateTableList.children("tbody").children("tr").each(function(){
      	  		if($(this).children("td:nth-child(2)").text() == name){
      	  			dbClickHeighlight = false;
      	  			$(this).addClass("tableHighlight");
      	  			$("#realTimeStateTable-div").scrollTop(0);
      	  			$("#realTimeStateTable .tableHighlight").insertBefore($realTimeStateTableList.children("tbody").children("tr:first-child"));
      	  		}
      	  	});
      	  	//序号重新进行排序
      	  	var index = 1;
        ...
        //列表高亮
        tableHighlightBlue: function(name){
        	$realTimeStateTableList.children("tbody").children("tr").removeClass("tableHighlight-blue");
        	$realTimeStateTableList.children("tbody").children("tr").removeClass("tableHighlight");
      	  	$realTimeStateTableList.children("tbody").children("tr").each(function(){
      	  		if($(this).children("td:nth-child(2)").text() == name){
      	  			...
      	  			$(this).addClass("tableHighlight-blue");
      	  			$("#realTimeStateTable-div").scrollTop(0);
      	  			$("#realTimeStateTable .tableHighlight-blue").insertBefore($realTimeStateTableList.children("tbody").children("tr:first-child"));
      	  		}
      	  	});
      	  	//序号重新进行排序
      	  	var index = 1;
        ...
        //报警设置取消
        alarmSettingCancelClick: function(){
        	realTimeMonitoring.getAlarmSetting();
        	$("#alarmSettingInfo").modal("hide");
        },
        //车辆树右键菜单
        zTreeShowRightMenu: function(event, treeId, treeNode){
		    if (treeNode && !treeNode.noR && treeNode.type == "vehicle"&&treeNode.iconSkin!="vehicleSkin") {
				zTree.selectNode(treeNode);
				realTimeMonitoring.showRMenu(treeNode.id, event.clientX, event.clientY);
			}
        },
        showRMenu: function(type, x, y){
            editUrlPath="/clbs/m/basicinfo/equipment/simcard/proofreading_"+type;
            $("#rMenu").html('<a href= "'+editUrlPath+'" data-toggle="modal" data-target="#commonWin">获取SIM卡信息</a>')
			$("#rMenu ul").show();
            // @{/m/basicinfo/equipment/simcard/proofreading_vNumber}
			rMenu.css({"top":(y)+"px", "left":x+"px", "visibility":"visible"});
			$("body").bind("mousedown", realTimeMonitoring.onBodyMouseDown);
		},
		hideRMenu: function(){
			if (rMenu) rMenu.css({"visibility": "hidden"});
			$("body").unbind("mousedown", realTimeMonitoring.onBodyMouseDown);
		},
    ...
    $(function(){
        ...
        getId_ztree('treeDemo','search_condition','vehicle');
        //默认地图显示
        ...
        //报警设置取消
        $("#alarmSettingCancel").on("click",realTimeMonitoring.alarmSettingCancelClick);
         //报警参数设置
